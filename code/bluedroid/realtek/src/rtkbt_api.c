...
int rtkbt_api_SetUipcPath(int ch_id, char * path)
{
    ...
    if(buffer == NULL)
    {
        BTIF_TRACE_ERROR1("%s GKI_getbuf return NULL", __FUNCTION__);
        return -1;
    }
    strcpy(buffer, path);
...
{
    int res;
    BTIF_TRACE_API4("%s(%d,%d,*,%d)", __FUNCTION__, id, command, len);
    if((id<0)||(id>=RTKBT_MAX_ID))
    {
        ALOGE("%s id(%d) is out of range,RTKBT_MAX_ID=%d\n", __FUNCTION__, id, RTKBT_MAX_ID);
        return -9998;
...
{
    int res = 0;
    BTIF_TRACE_API2("%s(%d)", __FUNCTION__, id);
    if((id<0)||(id>=RTKBT_MAX_ID))
    {
        ALOGE("%s id(%d) is out of range,RTKBT_MAX_ID=%d\n", __FUNCTION__, id, RTKBT_MAX_ID);
        return 0;
...
static void rtkbt_SendGenericEvent(char* p_param)
{
    ...
    event = *p++;
    len = *p++;
    BTIF_TRACE_ERROR4("rtkbt_SendGenericEvent(%d,%d,%08x,%d)\n", id, event, p, len);
...
{
    BTIF_TRACE_ERROR2("rtkbt_SendEvent(%d,%08x)\n", event, p_param);
    switch(event)
    {
        case EVENT_GENERIC:
...
int rtkbt_api_SendGenericEvent(int id, int event, unsigned char * data, int len)
{
    ...
    BTIF_TRACE_API4("rtkbt_api_SendGenericEvent(%d,%d,%08x,%d)\n", id, event, data, len);
    if(len > 1000)
    {
        BTIF_TRACE_ERROR4("rtkbt_api_SendGenericEvent(%d,%d,%08x,%d) len too long\n", id, event, data, len);
        return -1;
    }
    if(rtkbt_callbacks && rtkbt_callbacks->onGenericEvent)
    {
        ...
        *ip++ = len;
        ...
        BTIF_TRACE_ERROR4("rtkbt_api_SendGenericEvent(%d,%d,%08x,%d) rtkbt_callbacks == NULL or onGenericEvent==NULL\n", id, event, data, len);
...
void loadplugins(RTKBT_API_CALLBACK * callback)
{
    ...
    if(!fp)
    {
        BTIF_TRACE_ERROR1("fopen %s fail",RTKBT_CONF);
        return;
    }
    ...
    while(fgets(plugin, sizeof(plugin)-1, fp) != NULL)
    {
        ...
        BTIF_TRACE_API1("loadconfig(%s) ... ", path);
        rtkbt_loadconfig(path);
        sprintf(path, "%s/%s.so", RTKBT_PLUGIN_DIR, plugin);
        BTIF_TRACE_API1("loadplugins(%s) ... ", path);
        handle = dlopen(path, RTLD_NOW);
        if(!handle){
            BTIF_TRACE_ERROR1("dlopen(%s) fail!", path);
            continue;
        }
        ...
        if(plugin_info)
        {
            if((plugin_info->version&0xffff0000) == (RTKBT_API_VERSION&0xffff0000)){
                BTIF_TRACE_API3("loadplugin(%s) OK: Plugin Version(%08x) RTKBT_API_VERSION(%08x)", path, plugin_info->version, RTKBT_API_VERSION);
                plugin_info->init(callback, &arg);
            }else
                BTIF_TRACE_ERROR3("loadplugin(%s) fail: Plugin Version(%08x) RTKBT_API_VERSION(%08x)", path, plugin_info->version, RTKBT_API_VERSION);
        }else
            BTIF_TRACE_ERROR1("loadplugin(%s) fail:dlsym(PLUGIN_INFO) fail!", path);
    }
    if(fp)
        fclose(fp);
...
{
    BTIF_TRACE_API2("%s RTKBT_API_VERSION: %08x", __FUNCTION__, RTKBT_API_VERSION);
