        ...
        info("handleMessage:"+msg.what);
            switch (msg.what) {
                ...
                case RTKBT_AUTOPAIR_Event_ClassicRemoveBondResult:
                    ...
                    RTKBT_AUTOPAIR_pending_device.createBond();
                    break;
...
*******************************************************************************************************/
    private void onLeScanResult(BluetoothDevice device, int rssi, byte[] scanRecord){
...
// Device scan callback.
	private BluetoothAdapter.LeScanCallback mLeScanCallback =
	    ...
	    public void onLeScan(BluetoothDevice device, int rssi, byte[] scanRecord) {
	        info("mLeScanCallback"+device.getAddress());
			onLeScanResult(device, rssi, scanRecord);
	    }
	};
...
*************************************************************************************************/
    private synchronized int checkAndRestartScan( boolean enable)
    {
        ...
        if(RTKBT_IS_scan == false) {
			...
			RTKBT_AUTOPAIR_BluetoothAadapter.startLeScan(mLeScanCallback);
            RTKBT_IS_scan = true;
...
******************************************************************************************************/
  ...
  /**************************************************************************************************************
    ...
    ***************************************************************************************************************/
    private boolean match_vendor_data(byte[] scanRecord) {
        int j =0;
        int length =0;
...
**      args     : auto_scan if true the scan will auto with autopair vendor event, else scan will begin depending
                   on user.
***************************************************************************************************************/
    private boolean create_bond_by_type(BluetoothDevice device,byte[] scanRecord, boolean auto_scan) {
        ...
        for(int i = 0; i < mLoadConfig.total_type_num; i++){
            ...
            switch(autopair_type_temp) {
                ...
                {
                    info("key pair");
                    if(match_key(scanRecord, is_pair))
                        return true;
                }
                break;
